...
export class PlayerCage {
    ...
    static getCagePos(_data: CarData): Vector3 {
        ...
        const carEntityTransform = Transform.get(_data.carEntity)
        const playerCageTransform = Transform.get(_data.playerCageEntity)
    ...
    static updatePlayerCage(_dt: number, _data: CarData): void {
        if (!_data.playerCageEntity || !Transform.getMutable(_data.playerCageEntity)) return

        const cageScale = Transform.getMutable(_data.playerCageEntity).scale
        ...
        Transform.getMutable(_data.playerCageEntity).scale = Vector3.create(1 / newScale.x, 1 / newScale.y, 1 / newScale.z)
    }
    ...
    static expandCage(_data: CarData): void {
        ...
        Transform.getMutable(_data.playerCageEntity).scale = Vector3.create(1 / scale.x, 1 / scale.y, 1 / scale.z)
    }
}